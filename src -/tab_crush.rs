...
impl CrushTab {
    ...
    pub unsafe fn refresh_dropdown_label(&mut self, hz: u32, v: i32) {
...
SendMessageW(self.h_ddl_refresh, WM_SETREDRAW, WPARAM(1), LPARAM(0));
    ...
    ///
    /// Returns `(true, v)` only when the ramp was actually accepted by the driver.
    /// Some GPU drivers (e.g. NVIDIA with Colour Accuracy Mode / HDR) return success
    /// from `SetDeviceGammaRamp` but silently ignore the write.  We detect this by
    /// reading the ramp back immediately after writing and comparing — if the hardware
    /// value doesn't match what we wrote, the driver blocked it and we return `false`.
    pub unsafe fn apply_ramp(&self) -> (bool, i32) {
        let v    = get_slider_val(self.h_sld_black);
        let ramp = gamma_ramp::build_ramp(v);
        let null = HWND(std::ptr::null_mut());
        let hdc  = GetDC(null);
        if hdc.0.is_null() { return (false, v); }
        let set_ok = gamma_ramp::SetDeviceGammaRamp(hdc, &ramp) != 0;
        // Read back to detect drivers that silently ignore the write.
        let accepted = if set_ok {
            let mut actual = gamma_ramp::GammaRamp { red: [0; 256], green: [0; 256], blue: [0; 256] };
            let get_ok = gamma_ramp::GetDeviceGammaRamp(hdc, &mut actual) != 0;
            get_ok && actual == ramp
        } else {
            false
        };
